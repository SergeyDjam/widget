...
Функция ПолучитьНаКоординатыжатогоВиджета(ДанныеСобытия, byX, byY) Экспорт
	...
	Координаты.Вставить("left", Цел(byX * ДанныеСобытия.clientX / ЭтотОбъект.HTMLWidth));
	Координаты.Вставить("top", Цел(byY * ДанныеСобытия.clientY / ЭтотОбъект.HTMLHeight));
	Возврат Координаты;
КонецФункции
...
Процедура ЗагрузкаМакетов() Экспорт
	...
	svgjs = ПолучитьМакет("svgjs").ПолучитьТекст();
КонецПроцедуры	
...
Процедура ЗагрузитьДанныеИзСтруктуры( Структура, widgets_table ) Экспорт
	...
	Если ТипЗнч(Структура) = Тип("Структура") Тогда
		...
		Если Структура.Свойство("code1C") Тогда
			code1C = Структура.code1C;
		КонецЕсли;
		Если Структура.Свойство("RefLibs") Тогда
			RefLibs = Структура.RefLibs; 					
		КонецЕсли;
		Если Структура.Свойство("codeHTML") Тогда
			codeHTML = Структура.codeHTML; 					
		КонецЕсли;
...
Функция ВыбратьИзБиблиотеки() Экспорт 
	...
	HttpЗапрос = Новый HTTPЗапрос("/_"+script_name+".php?email="+online_email+"&offset="+Формат(widget_select,"ЧДЦ=; ЧН=0; ЧГ=0"), Заголовки);
	мОтвет = HttpСоединение.Получить(HttpЗапрос);
	Если мОтвет.КодСостояния = 200 Тогда
		СтрокаJSON = мОтвет.ПолучитьТелоКакСтроку(КодировкаТекста.UTF8);
...
Функция ПолучитьДанныеВСтрокуJSON() Экспорт
	...
	Структура.Вставить("RefLibs", RefLibs);
	Структура.Вставить("codeHTML", codeHTML);
...
Функция НарисоватьВиджеты(widgets_table, byX = 1, byY = 1, РисоватьСетку = Ложь, mode_select = Ложь)  Экспорт
	...
	Если widgets_table.Количество() > 0 Тогда
		...
		Для каждого widget из widgets_table Цикл
			...
			КонецЕсли;	
			fSelect = (widget_select = (widget.НомерСтроки));
			html_widget = НарисоватьОдинВиджет( widget.code1C, widget.codeDraw, x, y, w, h, widget.onServer, fSelect);
	...
	КонецЕсли;	
	html_view = СтрЗаменить(html_view, "<!-- codeHTML -->", codeHTML);
	html_view = СтрЗаменить(html_view, "<!-- RefLibs -->", RefLibs);
...
Функция НарисоватьОдинВиджет(code1C, codeDraw, x="0", y="0", w="(width)", h="(height)", onServer = false, fSelect = false, singleView= false, libs="" ) Экспорт
